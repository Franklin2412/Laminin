...
public class VectorDrawableCompat extends VectorDrawableCommon {
    ...
    private static class VectorDrawableCompatState extends ConstantState {
        ...
        public boolean canReuseBitmap(int i, int i2) {
            return (i == this.mCachedBitmap.getWidth() && i2 == this.mCachedBitmap.getHeight()) ? true : VectorDrawableCompat.DBG_VECTOR_DRAWABLE;
        }

        public boolean canReuseCache() {
            return (!this.mCacheDirty && this.mCachedTint == this.mTint && this.mCachedTintMode == this.mTintMode && this.mCachedAutoMirrored == this.mAutoMirrored && this.mCachedRootAlpha == this.mVPathRenderer.getRootAlpha()) ? true : VectorDrawableCompat.DBG_VECTOR_DRAWABLE;
        }
    ...
    private void printGroupTree(VGroup vGroup, int i) {
        String str = BuildConfig.VERSION_NAME;
        for (int i2 = LINEJOIN_MITER; i2 < i; i2 += LINEJOIN_ROUND) {
            str = str + "    ";
        }
        ...
        for (int i3 = LINEJOIN_MITER; i3 < vGroup.mChildren.size(); i3 += LINEJOIN_ROUND) {
            Object obj = vGroup.mChildren.get(i3);
            if (obj instanceof VGroup) {
    ...
    public boolean isStateful() {
        return this.mDelegateDrawable != null ? this.mDelegateDrawable.isStateful() : (super.isStateful() || !(this.mVectorState == null || this.mVectorState.mTint == null || !this.mVectorState.mTint.isStateful())) ? true : DBG_VECTOR_DRAWABLE;
    }
    ...
    PorterDuffColorFilter updateTintFilter(PorterDuffColorFilter porterDuffColorFilter, ColorStateList colorStateList, Mode mode) {
        return (colorStateList == null || mode == null) ? null : new PorterDuffColorFilter(colorStateList.getColorForState(getState(), LINEJOIN_MITER), mode);
    }
}
